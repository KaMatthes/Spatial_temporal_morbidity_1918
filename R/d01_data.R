...
dt <- rbind(dt.1918,dt.1919,dt.1920) %>%
  ...
  mutate(Startdatum = ymd(Startdatum),
         ...
         Gemeindename = ifelse(Bezirksname=="Appenzell-Innerrhoden","Bezirk", Gemeindename ),
...
dt3 <- dt %>%
  ...
  mutate(Cases = ifelse(Fallzahl_t>fallzahl_bezirk,Fallzahl_t, fallzahl_bezirk),
         Kanton = ifelse(Bezirksname=="Appenzell-Innerrhoden","AI", Kanton ),
         Kanton = ifelse(Bezirksname=="Genève","GE", Kanton ))

write.xlsx(dt3 ,"data/Faelle_Bezirke.xlsx", row.names=FALSE, overwrite = TRUE)
