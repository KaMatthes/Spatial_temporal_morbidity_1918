...
dt <- read.xlsx("data/cases_district_weekly.xlsx", detectDates = TRUE) %>%
  group_by(Canton,Report_start) %>%
  mutate(
    Cases_canton = sum(Cases),
    Pop_canton = sum(pop)
    ) %>%
  ungroup() %>%
  mutate(
  ...
  distinct(Canton,Report_start,.keep_all = TRUE) 

dt_t <- dt %>%
  group_by(Report_start) %>%
  mutate(
