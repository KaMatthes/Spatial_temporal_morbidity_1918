...
dt <- read.xlsx("output/Regression_rlm_zscore.xlsx") %>%
  filter(!Cofactor %in% c("hospitals","Hospitals per 10'000","Doctors per 10'000","Hospital doctors per 10'000")) %>%
  mutate(wave = as.factor(wave),
         ...
         Cofactor = recode(Cofactor,"Private doctors per 10'000"="Doctors per 10'000"),
...
ggplot(dt, aes(x=Cofactor,ymin=Cl, ymax=Cu,y=est, col=col_plot),show.legend = FALSE) + 
...
ggsave("figures/Figure_incidence_regression2.png",h=15,w=18)
